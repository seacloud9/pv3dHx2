 ...
 * @author Tim Knip
 */
class KMZ extends TriangleMesh3D {
	 ...
	 * Constructor.
	 */
	public function new(name:String=null):Void {
	 ...
	 */
	public function load(asset:Dynamic, materials:MaterialsList=null):Void {
		...
		if(Std.is(asset, String)){
			...
			throw new Dynamic("KMZ#load:don't know how to load asset:" + asset);
		}
	}
	...
	private function getColladaFromZip(zipFile:ZipFile):ByteArray  {
		for(i in 0...zipFile.entries.length){
			var entry:ZipEntry=zipFile.entries[i];
		
			// extract the entry's data from the zip
			var data:ByteArray<Dynamic>=zipFile.getInput(entry);
	...
	private function onTextureComplete(event:Event=null):Void {
		if(event && event.target is Bitmap){
			...
			for(var effect:XML in effects){
				try {
					var id:String=effect.@id.toString();
					var images:XMLList=effect..collada::init_from;
			
					for(var image:XML in images){
						var init_from:String=Std.string(image.text());
						...
						if(img_url.indexOf(loader.name)!=-1){
							...
							this.materials.addMaterial(material, Std.string(mat.@id));
						}
					}
				} catch(e:Dynamic){
	...
	private function parse(data:ByteArray):Void {
		...
		for(i in 0...zipFile.entries.length){
			...
			// extract the entry's data from the zip
			var dataInput:ByteArray<Dynamic>=zipFile.getInput(entry);
	 ...
	 */
	private function numTexturesInZip(zipFile:ZipFile):Int {
		var count:Int=0;
		for(i in 0...zipFile.entries.length){
			var entry:ZipEntry=zipFile.entries[i];
	...
	/** */
	private var _loadedTextures:Int;
	
	/** */
	private var _totalTextures:Int;
	
	/** */
	private var _loadedDAE		:ByteArray<Dynamic>;
}