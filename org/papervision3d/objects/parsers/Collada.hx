...
*</li></ul>
*/
class Collada extends DisplayObject3D
{
	/**
	* Default scaling value for constructor.
	*/
	static public var DEFAULT_SCALING:Float=1;
	
	/**
	* Internal scaling value.
	*/
	static private var INTERNAL_SCALING:Float=100;
	...
	*/
	...
	public function new(COLLADA:Dynamic=null, materials:MaterialsList=null, scale:Float=1, doubleSided:Bool=false)
	{
		super(null, null);
	...
	* @param	doubleSided		Whether to make all materials doubleSided.
	*/ 
	public function load(COLLADA:Dynamic, materials:MaterialsList=null, scale:Float=1, doubleSided:Bool=false):Void
	{
		...
		this._geometries=new Dynamic();

		if(Std.is(COLLADA, XML))
		{
			this.COLLADA=COLLADA;
			this._filename="";
			buildCollada();
		}
		else if(Std.is(COLLADA, String))
		{
	...
	{
		for(var node:XML in scene.node)
		...
		for(i in 0...totalChildren)
		{
			...
			switch(child.name().localName)
			{
				...
				case "instance_geometry":
					...
					// Instance materials
					var bindMaterial:Dynamic=new Dynamic();
					for(var instance_material:XML in child..instance_material)
					{
						bindMaterial[ instance_material.@symbol ]=instance_material.@target.split("#")[1];
					}

					// Instance geometry
					for(var geometry:XML in child)
					{
	...
	private function parseGeometry(geometry:XML, instance:DisplayObject3D, matrix2:Matrix3D=null, bindMaterial:Dynamic=null):Void
	{
		...
		// Semantics
		var semantics:Dynamic=new Dynamic();
		semantics.name=geometry.@id;

		var faces:Array<Dynamic>=semantics.triangles=new Array();

		// Multi material
		var multiMaterial:Bool=(geometry.mesh.triangles.length()>1);

		// Triangles
		for(var triangles:XML in geometry.mesh.triangles)
		{
			// Input
			var field:Array<Dynamic>=new Array();

			for(var input:XML in triangles.input)
			{
			...
			for(j in 0...len)
			{
				var t:Dynamic=new Dynamic();

				for(v in 0...3)
				{
					var fld:String;
					for(var k:Float=0;fld=field[k];k++)
					{
	...
	private function buildObject(semantics:Dynamic, instance:DisplayObject3D, matrix:Matrix3D=null):Void
	{
		...
		var scaling:Float=this._scaling;
		var accVerts:Float=vertices.length;

		var semVertices:Array<Dynamic>=semantics.VERTEX;
		var len:Float=semVertices.length;
		...
		for(i=0;i<len;i++)
		{
			...
			var vert:Dynamic=semVertices[ i ];
			var x:Float=Std.parseFloat(vert.X)* scaling;
			var y:Float=Std.parseFloat(vert.Y)* scaling;
			var z:Float=Std.parseFloat(vert.Z)* scaling;
		...
		// Faces
		var faces	:Array<Dynamic>=instance.geometry.faces=new Array();
		var semFaces:Array<Dynamic>=semantics.triangles;
		len=semFaces.length;
	
		for(i=0;i<len;i++)
		{
			// Triangle
			var tri:Array<Dynamic>=semFaces[i].VERTEX;
			var a:Vertex3D=vertices[ accVerts + tri[ 0 ] ];
			var b:Vertex3D=vertices[ accVerts + tri[ 1 ] ];
			var c:Vertex3D=vertices[ accVerts + tri[ 2 ] ];

			var faceList:Array<Dynamic>=[ a, b, c ];

			var tex:Array<Dynamic>=semantics.TEXCOORD;
			var uv:Array<Dynamic>=semFaces[i].TEXCOORD;

			var uvList:Array, uvA:FloatUV, uvB:FloatUV, uvC:FloatUV;

			if(uv && tex)
			{
				uvA=new FloatUV(tex[ uv[0] ].S, tex[ uv[0] ].T);
				uvB=new FloatUV(tex[ uv[1] ].S, tex[ uv[1] ].T);
				uvC=new FloatUV(tex[ uv[2] ].S, tex[ uv[2] ].T);
				uvList=[ uvA, uvB, uvC ];
			}
			else uvList=null;
	...
	private function getArray(spaced:String):Array
	{
		...
		var numbers:Array<Dynamic>=new Array();

		var totalStrings:Float=strings.length;

		for(i in 0...totalStrings)
			numbers[i]=Std.parseFloat(strings[i]);

		return numbers;
	}


	private function addMaterial(instance:DisplayObject3D, name:String, bindMaterial:Dynamic):Void
	{
	...
	private function deserialize(input:XML, geo:XML):Array
	{
		...
		if(acc !=new XMLList(null))
		{
			...
			var floats:Array<Dynamic>=floStr.split(" ");

			// Build params array
			var params:Array<Dynamic>=new Array();

			for(var par:XML in acc.param)
			...
			for(i in 0...count)
			{
				var element:Dynamic=new Dynamic();
	...
	private var _doubleSidedMaterials:Bool=false;
