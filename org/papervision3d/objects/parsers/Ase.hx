...
*/
class Ase extends TriangleMesh3D
{
	/**
	...
	*/
	static public var INTERNAL_SCALING:Float=50;
	...
	*<p/>
	*/
	public function new(material:MaterialObject3D, filename:String, scale:Float=1)
	{
	...
	*/
	...
	{
		var scale:Float=this._scaleAse;
		scale *=INTERNAL_SCALING;

		var vertices:Array<Dynamic>=this.geometry.vertices=new Array();
		var faces	:Array<Dynamic>=this.geometry.faces=new Array();

		var loader:URLLoader=URLLoader(e.target);
		var lines:Array<Dynamic>=unescape(loader.data).split('\r\n');
		...
		while(lines.length)
		{
			...
			switch(chunk)
			{
				case 'MESH_VERTEX_LIST':
					try
					{
						while((content=Std.string(lines.shift())).indexOf('}')<0)
						{
							content=content.split("*")[1];

							//content=content.split("	").join("\t");
							var mvl:Array<Dynamic>=content.split('\t');// separate here

							var x:Float=Std.parseFloat(mvl[1])* scale;
							var y:Float=Std.parseFloat(mvl[3])* scale;
							var z:Float=Std.parseFloat(mvl[2])* scale;// Swapped Y and Z
				...
				case 'MESH_FACE_LIST':
					try
					{
						while((content=Std.string(lines.shift())).indexOf('}')<0)
						{
							...
							var con:String;
							con=drc[2];
							var a:Vertex3D=vertices[ Std.int(con.substr(0, con.lastIndexOf(' ')))];

							con=drc[3];
							var b:Vertex3D=vertices[ Std.int(con.substr(0, con.lastIndexOf(' ')))];

							con=drc[4];
							var c:Vertex3D=vertices[ Std.int(con.substr(0, con.lastIndexOf(' ')))];

							// Swap b/c
							faces.push(new Triangle3D(this,[a, b, c], null, [new FloatUV(), new FloatUV(), new FloatUV()]));
						}
					}
					catch(err:Dynamic)
					{
				...
				case 'MESH_TVERTLIST':
					try
					{
						while((content=Std.string(lines.shift())).indexOf('}')<0)
						{
							...
							var mtvl:Array<Dynamic>=content.split('\t');// separate here
							uvs.push(new FloatUV(parseFloat(mtvl[1]), parseFloat(mtvl[2])));
						}
					}
					catch(errorMesh:Dynamic)
					{
				...
				case 'MESH_TFACELIST':
					try
					{
						...
						while((content=Std.string(lines.shift())).indexOf('}')<0)
						{
							...
							var mtfl:Array<Dynamic>=content.split('\t');// separate here

							var faceUV:Array<Dynamic>=faces[ num ].uv;
	...
	private function ioErrorHandler(event:IOErrorEvent):Void
	{
		...
		dispatchEvent(fileEvent);
		throw new Dynamic("Ase:ioErrorHandler Dynamic.");
	}
