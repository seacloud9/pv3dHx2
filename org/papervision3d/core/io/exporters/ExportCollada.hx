...
class ExportCollada
{
	...
	/** Float of fraction digits to use for floats. */
	public static var FRACTION_DIGITS:Int=5;
	
	/** Bool indicatin whether to flip faces. */
	public static var REVERSE_WINDING:Bool=true;
	...
	{
		var rgba:Array<Dynamic>=new Array();
		
		if(material)
		{
			var color:Int=material is WireframeMaterial ? material.lineColor:material.fillColor;
	...
	private static function exportEffects(object:DisplayObject3D, indent:Int=0):String 
	{
		...
		for(name in object.materials.materialsByName)
		{
		...
		for(var child:DisplayObject3D in object.children)
	...
	private static function exportGeometries(instance:DisplayObject3D, indent:Int=0):String
	{
		...
		if(Std.is(instance, TriangleMesh3D))
	...
	private static function exportGeometry(mesh:TriangleMesh3D, id:String, indent:Int=0):String 
	{
		...
		var tri:Triangle3D;
		var v:Vertex3D;
		var trianglesByMaterial:Dynamic=new Dynamic();
		var uvs:Array<Dynamic>=new Array();
		var vData:Array<Dynamic>=new Array();
		var uvData:Array<Dynamic>=new Array();
		...
		// export a<triangles>element for material used by the geometry
		for(materialName in trianglesByMaterial)
		{
			var tris:Array<Dynamic>=trianglesByMaterial[materialName];
			var p:Array<Dynamic>=new Array();
			
			for(i=0;i<tris.length;i++)
			{
				tri=mesh.geometry.faces[ tris[i] ];
				
				var uva:Array<Dynamic>=uvindices[ tri ];
	...
	private static function exportImages(object:DisplayObject3D, indent:Int=0):String 
	{
		...
		for(name in object.materials.materialsByName)
		{
			...
			if(Std.is(material, BitmapFileMaterial))
			{
				...
				if(url.indexOf("/")!=-1)
				{
					var parts:Array<Dynamic>=url.split("/");
					url=Std.string(parts.pop());
				}
	...
	{				
		...
		if(Std.is(object, TriangleMesh3D))
		{
			...
			for(materialName in object.materials.materialsByName)
			{
	...
	private static function prepareMaterials(object:DisplayObject3D):Void
	{
		...
		{
			for(var triangle:Triangle3D in object.geometry.faces)
			{
	...
	{
		for(name in list.materialsByName)
		{
	...
	private static var _hasImages			:Bool;
