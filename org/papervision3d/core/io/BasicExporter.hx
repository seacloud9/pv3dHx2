...
class BasicExporter
{
	 ...
	 * @return ByteArray
	 */ 
	public static function export(object:DisplayObjectContainer3D, exportFileFormat:Int=0):ByteArray
	{
		var ba:ByteArray<Dynamic>=new ByteArray();
		var root:DisplayObject3D;
		
		if(Std.is(object, SceneObject3D))
		{
			var scene:SceneObject3D=cast(object, SceneObject3D);
			root=new DisplayObject3D("SceneObjectNode");
			for(i in 0...scene.objects.length)
				root.addChild(scene.objects[i]);
		}
		else if(Std.is(object, DisplayObject3D))
		{
			root=cast(object, DisplayObject3D);	
		}
		else
		{
	...
	public static function test():Void
	{
		...
		var ba:ByteArray<Dynamic>=export(scene, ExportFileFormat.COLLADA);
