 ...
 */
class Mouse3D extends DisplayObject3D
{
	static private var UP 								:Float3D=new Float3D(0, 1, 0);
	...
	//private var _position:Float3D=new Float3D(0, 0, 0);
	private var target:Float3D=new Float3D();
	...
	{			
		...
		var zAxis:Float3D=Number3D.sub(target, position);
		zAxis.normalize();
			
		if(zAxis.modulo>0.1)
		{
			var xAxis:Float3D=Number3D.cross(zAxis, UP);
			xAxis.normalize();
			
			var yAxis:Float3D=Number3D.cross(zAxis, xAxis);
			yAxis.normalize();
