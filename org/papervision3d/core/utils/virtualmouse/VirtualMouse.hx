...
class VirtualMouse extends EventDispatcher {
	...
	private var disabledEvents:Dynamic=new Dynamic();
	private var ignoredInstances:Dictionary=new Dictionary(true);
	
	private var _lastEvent:Event;
	private var lastMouseDown:Bool=false;
	private var updateMouseDown:Bool=false;
	private var lastLocation:Point;
	private var lastDownTarget:DisplayObject;
	private var lastWithinStage:Bool=true;
		
	private var _useNativeEvents:Bool=false;
	...
	private function set_stage(s:Stage):Void {
		var hadStage:Bool;
		if(_stage){
			hadStage=true;
	...
	* @return
	*/
	private function set_container(value:Sprite):Void
	{
		_container=value;
	}
	 ...
	 */
	public var lastEvent(get_lastEvent, set_lastEvent):Event;
	 ...
	 * @see release()
	 */
	public var mouseIsDown(get_mouseIsDown, set_mouseIsDown):Bool;
	...
	public var x(null, set_x):Float;
 	private function set_x(n:Float):Void {
		location.x=n;
		if(!isLocked)update();
	}
	...
	public var y(null, set_y):Float;
 	private function set_y(n:Float):Void {
		location.y=n;
		if(!isLocked)update();
	}
	...
	private function set_useNativeEvents(b:Bool):Void {
	 ...
	 * Point instance, or by passing in two Float
	 * instances representing x and y coordinates.
	 * The location of the virtual mouse is based in
	 * the global coordinate space.
	 * @param a A Point instance or x Float value.
	 * @param b A y Float value if a is a Float.
	 * @see x
	 * @see y
	 * @see getLocation()
	 */
	public function setLocation(a:Dynamic, b:Dynamic=null):Void 
	{
		...
		if(Std.is(a, Point)){
			var loc:Point=cast(a, Point);
			location.x=loc.x;
			location.y=loc.y;
		}else{
			location.x=Std.parseFloat(a);
			location.y=Std.parseFloat(b);
		}
	...
	private function handleUpdate(event:Event):Void 
	{
		...
		//var objectsUnderPoint:Array<Dynamic>=container.getObjectsUnderPoint(p);
		...
		var objectsUnderPoint:Array<Dynamic>=container.getObjectsUnderPoint(location);
