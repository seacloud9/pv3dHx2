...
class RenderPixels extends RenderableListItem implements IRenderListItem
{
	private var pixels:Pixels;
	public function new(pixels:Pixels)
	{
		this.pixels=pixels;
	...
	override public function render(renderSessionData:RenderSessionData, graphics:Graphics):Void
	{
		...
		var offsetX:Float=layer.width>>1;
		var offsetY:Float=layer.height>>1;
		var canvas:BitmapData=layer.canvas;
		
		var v3d:Vertex3DInstance;
		screenZ=0;
		
		for(var p:Pixel3D in pixels.pixels){
