...
class DisplayObjectContainer3D extends EventDispatcher
{
	...
	*/
	private var _childrenByName:Dynamic;
	...
	*/
	public function addChild(child:DisplayObject3D, name:String=null):DisplayObject3D
	{
		...
		// Choose name
		name=name || child.name || Std.string(child.id);
	...
	*/
	...
	{
		for(var child:DisplayObject3D in parent.children)
		{
	...
	*/
	public function getChildByName(name:String, recursive:Bool=false):DisplayObject3D
	{
		if(recursive)
	...
	*/
	public var children(get_children, null):Dynamic;
 	private function get_children():Dynamic
	{
