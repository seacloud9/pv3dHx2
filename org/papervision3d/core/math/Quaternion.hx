...
class Quaternion 
{
	...
	public static inline var DEGTORAD:Float=(Math.PI/180.0);
	public static inline var RADTODEG:Float=(180.0/Math.PI);
	 ...
	 * @return
	 */
	public function setFromAxisAngle(x:Float, y:Float, z:Float, angle:Float):Void
	{
	 ...
	 */ 
	public function setFromEuler(ax:Float, ay:Float, az:Float, useDegrees:Bool=false):Void
	{
		if(useDegrees)
		{
			ax *=DEGTORAD;
	 ...
	 * @return
	 */
	public static function createFromAxisAngle(x:Float, y:Float, z:Float, angle:Float):Quaternion
	{
		var q:Quaternion=new Quaternion();
	 ...
	 * @return
	 */
	public static function createFromEuler(ax:Float, ay:Float, az:Float, useDegrees:Bool=false):Quaternion
	{
		if(useDegrees)
		{
			ax *=DEGTORAD;
	...
	public static function createFromMatrix(matrix:Matrix3D):Quaternion
	{
		...
		var s:Float;
		var q:Array<Dynamic>=new Array(4);
		var i:Int, j:Int, k:Int;
		
		var tr:Float=matrix.n11 + matrix.n22 + matrix.n33;
	 ...
	 */	
	public static function slerp(qa:Quaternion, qb:Quaternion, alpha:Float):Quaternion
	{
		var angle:Float=qa.w * qb.w + qa.x * qb.x + qa.y * qb.y + qa.z * qb.z;
	 ...
	 * @return the Interpolated quaternion.
	 */
	public static function slerpOld(qa:Quaternion, qb:Quaternion, alpha:Float):Quaternion
	{
		var qm:Quaternion=new Quaternion();
		
		// Calculate angle between them.
		var cosHalfTheta:Float=qa.w * qb.w + qa.x * qb.x + qa.y * qb.y + qa.z * qb.z;
		...
		var ratioA:Float=Math.sin((1 - alpha)* halfTheta)/ sinHalfTheta;
		var ratioB:Float=Math.sin(alpha * halfTheta)/ sinHalfTheta;
	...
	public function toEuler():Float3D
	{
		var euler	:Float3D=new Float3D();
		var q1		:Quaternion=this;
		
		var test:Float=q1.x*q1.y + q1.z*q1.w;
		...
		var sqx	:Float=q1.x*q1.x;
		var sqy	:Float=q1.y*q1.y;
		var sqz	:Float=q1.z*q1.z;
