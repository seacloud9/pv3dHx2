 ...
 * 	 */	class FastRectangleTools 
{
	...
	public static function Intersects(rect1:Rectangle, rect2:Rectangle):Bool
	{
	 ...
	 */ 
	public static function getRotatedBounds(rect:Rectangle, angle:Float, targetrect:Rectangle=null):Rectangle
	{
		if(!targetrect)targetrect=new Rectangle();
		...
		var absSinA:Float=Math.abs(Math.sin(angle));
		var absCosA:Float=Math.abs(Math.cos(angle));
