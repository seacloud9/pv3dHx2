...
*
*/
class BitmapMaterial extends TriangleMaterial implements ITriangleDrawer
{
	
	protected static var DEFAULT_FOCUS:Float=200;
	protected static var hitRect:Rectangle=new Rectangle();
	
	
	private var renderRecStorage:Array<Dynamic>;
	private var focus:Float=200;
	private var _precise:Bool;
	private var _precision:Int=8;
	private var _perPixelPrecision:Int=8;
	public var minimumRenderSize:Float=4;
	 ...
	 */
	public static var AUTO_MIP_MAPPING:Bool=false;

	/**
	 * Levels of mip mapping to force.
	 */
	public static var MIP_MAP_DEPTH:Float=8;
	...
	*/
	public function new(asset:BitmapData=null, precise:Bool=false)
	{
	...
	private function createRenderRecStorage():Void
	{
	...
	*/
	...
	{			
		...
		else if(bitmap)
		{
			...
			var w:Float=bitmap.width * maxU;
			var h:Float=bitmap.height * maxV;
			var u0:Float=w * face3D.uv0.u;
			var v0:Float=h *(1 - face3D.uv0.v);
			var u1:Float=w * face3D.uv1.u;
			var v1:Float=h *(1 - face3D.uv1.v);
			var u2:Float=w * face3D.uv2.u;
			var v2:Float=h *(1 - face3D.uv2.v);
	...
	*/
	...
	{			
		if(bitmap)
		{
			...
			var w:Float=bitmap.width * maxU;
			var h:Float=bitmap.height * maxV;
			var u0:Float=w * tri.uv0.u;
			var v0:Float=h *(1 - tri.uv0.v);
			var u1:Float=w * tri.uv1.u;
			var v1:Float=h *(1 - tri.uv1.v);
			var u2:Float=w * tri.uv2.u;
			var v2:Float=h *(1 - tri.uv2.v);
	...
	private var tempPreGrp:Graphics;
	private var tempPreBmp:BitmapData;
	private var tempPreRSD:RenderSessionData;		private var tempTriangleMatrix:Matrix=new Matrix();
	private var a2:Float;
	private var b2:Float;
	private var c2:Float;
	private var d2:Float;

	private var dx:Float, dy:Float, d2ab:Float, d2bc:Float, d2ca:Float;
	
	private function renderRec(emMap:Matrix, v0:Vertex3DInstance, v1:Vertex3DInstance, v2:Vertex3DInstance, index:Float):Void
	{
		az=v0.z;
		bz=v1.z;
		cz=v2.z;
	...
	*/
	...
	private function correctBitmap(bitmap:BitmapData):BitmapData
	{
		...
		var levels:Float=1<<MIP_MAP_DEPTH;
		// this is faster than Math.ceil
		var bWidth:Float=bitmap.width  / levels;
		bWidth=bWidth==uint(bWidth)? bWidth:Int(bWidth)+1;
		var bHeight:Float=bitmap.height  / levels;
		bHeight=bHeight==uint(bHeight)? bHeight:Int(bHeight)+1;
		
		var width:Float=levels * bWidth;
		var height:Float=levels * bHeight;
	...
	private function extendBitmapEdges(bmp:BitmapData, originalWidth:Float, originalHeight:Float):Void
	{
	...
	*/
	 ...
	 */
	private function set_precise(boolean:Bool):Void
	{
		_precise=boolean;
	}
	
	public var precise(get_precise, set_precise):Bool;
 	private function get_precise():Bool
	{
		return _precise;
	}
	 ...
	 */
	private function set_precision(precision:Int):Void
	{
	 ...
	 */
	private function set_pixelPrecision(precision:Int):Void
	{
		_precision=precision*precision*1.4;
		_perPixelPrecision=precision;
	}
	
	public var pixelPrecision(get_pixelPrecision, set_pixelPrecision):Int;
 	private function get_pixelPrecision():Int
	{
	...
	* @private
	*/
	private function set_texture(asset:Dynamic):Void
	{
		if(Std.is(asset, BitmapData)==false)
		{
