 ...
 */
class ShadedMaterial extends TriangleMaterial implements ITriangleDrawer,IUpdateBeforeMaterial,IUpdateAfterMaterial
{
	...
	public function updateBeforeRender(renderSessionData:RenderSessionData):Void
	{
		...
		for(sod in shaderObjectData){
			...
			if(Std.is(shader, ILightShader)){
				var ls:ILightShader=cast(shader, ILightShader);
	...
	private function set_shaderCompositeMode(compositeMode:Int):Void
	{
