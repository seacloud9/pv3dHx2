 ...
 */	
class QuadrantRenderEngine extends BasicRenderEngine implements IRenderEngine
{
	...
	public var quadFilters:Array<Dynamic>=[];
	
	public static var CORRECT_Z_FILTER:Float=0x01;
	public static var QUAD_SPLIT_FILTER:Float=0x02;
	public static var ALL_FILTERS:Float=CORRECT_Z_FILTER + QUAD_SPLIT_FILTER;

	/**
	 * Creates and prepares all the objects and events needed for rendering
	 */
	public function new(type:Float=3):Void
	{
	...
	private function getLayerObjects(layers:Array):Array{
		...
		for(var vpl:ViewportLayer in layers){
	...
	/** @private */
	protected override function doRender(renderSessionData:RenderSessionData, layers:Array<Dynamic>=null):RenderStatistics
	{
		stopWatch.reset();
		stopWatch.start();
		...
		for(var qf:AbstractQuadrantFilter in quadFilters){
